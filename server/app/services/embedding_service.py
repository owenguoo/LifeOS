...
class EmbeddingService:
    ...
    async def create_video_embedding_task(
        ...
        try:
            print(
                f"   📁 Using file: {file_path}"
                if file_path
                else f"   🌐 Using URL: {video_url}"
            )
            ...
            if file_path:
                ...
                task = self.client.embed.task.create(
                    model_name="Marengo-retrieval-2.7", video_file=file_path
                )
            else:
                # For URL, use video_url parameter
                task = self.client.embed.task.create(
                    model_name="Marengo-retrieval-2.7", video_url=video_url
                )
    ...
    async def wait_for_embedding_completion(
        ...
        try:
            print(
                f"   ⏰ Waiting for task completion (checking every {sleep_interval}s, max {max_wait_time}s)..."
            )
            ...
            while time.time() - start_time < max_wait_time:
                ...
                if status_result:
                    ...
                    if status_result.status == "ready":
                        ...
                        logger.error(
                            f"Task {task.id} failed with status: {status_result.status}"
                        )
    ...
    async def retrieve_video_embeddings(
        self, task, embedding_options: Optional[List[str]] = None
        ...
        try:
            ...
            if task_result:
                ...
                print(
                    f"   🔍 Task result attributes: {[attr for attr in dir(task_result) if not attr.startswith('_')]}"
                )
    ...
    async def process_video_embedding_pipeline(
        ...
        try:
            ...
            if final_status != "ready":
                print(
                    f"❌ Step 2 failed: Embedding task failed with status: {final_status}"
                )
            ...
            embeddings = await self.retrieve_video_embeddings(
                task=task, embedding_options=embedding_options
            )
