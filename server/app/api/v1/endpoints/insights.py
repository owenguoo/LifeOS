...
async def get_recent_events(current_user: User = Depends(get_current_user)):
    """
    ...
    try:
        ...
        videos = await supabase_manager.get_user_videos(
            user_id=str(current_user.id), limit=5, offset=0
        )
        ...
        for i, video in enumerate(videos, 1):
            if video.get("detailed_summary"):
                timestamp = video.get(
                    "timestamp", video.get("datetime", "Unknown time")
                )
                summary_parts.append(
                    f"{i}. {timestamp}: {video['detailed_summary'][:100]}..."
                )

        overall_summary = (
            f"Recent activity summary ({len(videos)} events):\n"
            + "\n".join(summary_parts)
        )
...
async def get_daily_recap(current_user: User = Depends(get_current_user)):
    """
    ...
    try:
                    ...
                    try:
                        if isinstance(video[date_field], str):
                            video_date = datetime.fromisoformat(
                                video[date_field].replace("Z", "+00:00")
                            ).date()
                        else:
        ...
        sorted_videos = sorted(
            today_videos, key=lambda x: x.get("timestamp", x.get("datetime", ""))
        )
