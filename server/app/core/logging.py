...
def log_api_request(endpoint: str, method: str, user_id: str = None, **kwargs):
    ...
    extra_data = {"endpoint": endpoint, "method": method, "user_id": user_id, **kwargs}
...
def log_video_processing(event: str, video_id: str = None, **kwargs):
    ...
    extra_data = {"event": event, "video_id": video_id, **kwargs}
    logger.info(f"Video Processing: {event}", extra={"extra_data": extra_data})


def log_error(error: Exception, context: Dict[str, Any] = None):
    logger = logging.getLogger("lifeos.error")
    extra_data = {"error_type": type(error).__name__, "context": context or {}}
    logger.error(
        f"Error occurred: {str(error)}", exc_info=True, extra={"extra_data": extra_data}
    )
