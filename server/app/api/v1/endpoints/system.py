...
async def start_video_system(
    ...
    try:
        ...
        if not api_key:
            raise HTTPException(
                status_code=500, detail="TWELVELABS_API_KEY is not configured"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to start video system: {str(e)}"
        )
...
async def stop_video_system(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to stop video system: {str(e)}"
        )
...
async def end_video_system(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        return JSONResponse(
            content={
                ...
                "uptime_seconds": round(
                    asyncio.get_event_loop().time() - system_info["started_at"], 2
                )
                if system_info
                else 0,
            }
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to end video system: {str(e)}"
        )
...
async def system_health():
    ...
    try:
        ...
        return JSONResponse(
            content={
                "status": "healthy"
                if redis_connected and api_key_configured
                else "unhealthy",
                "redis_connected": redis_connected,
                "api_key_configured": api_key_configured,
                "running_systems": running_count,
                "active_users": list(_running_systems.keys())
                if _running_systems
                else [],
            }
        )
