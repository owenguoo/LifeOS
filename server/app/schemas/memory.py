...
class MemoryCreateRequest(BaseModel):
    ...
    content_type: str = Field(
        ..., description="Type of content: 'video', 'audio', 'text', 'interaction'"
    )
...
class MemorySearchRequest(BaseModel):
    ...
    content_types: Optional[List[str]] = Field(
        default=None, description="Filter by content types"
    )
    ...
    score_threshold: Optional[float] = Field(
        default=0.01, ge=0.0, le=1.0, description="Minimum similarity score"
    )
...
class ChatbotQueryRequest(BaseModel):
    ...
    confidence_threshold: Optional[float] = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Minimum confidence score for video match",
    )
